...
if ( $PaymentOption == "PayPal" )
{
	...
	$username = $_SESSION["CONVERTER_USERID"];
			...
			$sql = "update `user_tbl` set payed = 1,payed_day= '".$db->esc($payed_day)."',payed_fee= '".$db->esc($finalPaymentAmount)."'  where username = '".$db->esc($username)."' ";
			$result = $db -> Exec($sql);
			$db -> close();
...
<script>
alert("お支払が成立しました。お支払金額："+<?php echo $finalPaymentAmount;?>);
