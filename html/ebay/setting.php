<?php
	...
	if (isset($_GET["exec"]) && $_GET["exec"] == "2"){
		...
		$item_specifics_default = mysql_escape_string($item_specifics_default);
...
<script type="text/javascript">
  ...
  function doExec(id){
  	document.frm.action = "setting.php?exec="+id;
  	document.frm.submit();
  }
...
function addColumn(id){
	$('#dir-tbody1').append('<tr>'+
		...
		'<td><input type=button class="btnDelete" id="btnDelete" value="削除" onclick="removeList(this, 1);"></td>' +
		'</tr>');
}
</script>
<!-- /header -->
...
<h2><img src="img/title_setting.gif" alt="環境設定" /></h2>
...
<form action="setting.php" name="frm" method="post">




<p class="midashi">
こちらのページでは、それぞれログイン情報、ebay出力項目の設定ができます。
</p>

<?php if ($GLOBALS['announce']) { ?><div class="attention"><?php echo_h($GLOBALS['announce']); ?></div><?php } ?>

<h2 class="subtitle">ログイン時の情報</h2>
	...
	<div align="center">
		<input type="submit" onclick="doExec('2')" name="exec2" value="" class="btn_hozon" />
	</div>



<h2 class="subtitle" style="width:400px;float:left;">EBay関連情報設定</h2>
...
<h2 class="subtitle" style="width:400px;float:left;">説明文の設定</h2>
...
<h2 class="subtitle" style="width:400px;float:left;">商品詳細（Item Specifics）の設定</h2>
...
$db -> close();
?>
<form action="setting.php?exec=4" name="frm4" method="post">
