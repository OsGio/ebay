<?php
	...
	if (isset($_GET["exec"]) && $_GET["exec"] == "1") {
		...
		//メールアドレスで既存のユーザーか否かを確認し、既存なら更新、新規なら追加
		$rc = $db -> Exec("SELECT * FROM user_tbl WHERE username='".$db->esc($username)."'");
		if($db -> NumRows($rc) > 0) {	
			...
			$hasUp = false;
			if($username <> "") {
				$sql = $sql . "username= '".$db->esc($username)."',";
				$hasUp = true;
			}			
			if($company_name <> "") {
				$sql = $sql . "company_name= '".$db->esc($company_name)."',";
			...
			if ($hasUp) {
				$sql = $sql . "create_time= '".date('Y-m-d H:i:s')."'";
				$db -> Exec($sql);
			}			
...
<table>
	...
	<td>
		...
		$db -> close();
		?>
		<tr><th class="lineNone">ユーザID</th><td><input type="text" name = "userid" value="<?php echo_h($username);?>" <?php if ($_REQUEST['username']) {echo 'readonly="readonly"';} ?>></td></tr>
		...
		<tr><th class="lineNone">eBayアカウント</th><td><input type="text" name = "ebay_id" value="<?php echo_h($ebay_id);?>"></td></tr>		
		<tr>
